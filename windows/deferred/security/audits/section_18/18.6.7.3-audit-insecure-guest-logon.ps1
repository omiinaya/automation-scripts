<#
.SYNOPSIS
    CIS Audit Script for 18.6.7.3 - Ensure 'Audit insecure guest logon' is set to 'Enabled'
.DESCRIPTION
    This script audits the insecure guest logon audit setting to ensure it is enabled.
    The setting checks HKLM\SOFTWARE\Policies\Microsoft\Windows\LanmanServer:AuditInsecureGuestLogon
.NOTES
    CIS ID: 18.6.7.3
    Profile: L1
    File Name: 18.6.7.3-audit-insecure-guest-logon.ps1
...
$registryValueName = "AuditInsecureGuestLogon"

# Custom audit script block for insecure guest logon audit checking
$auditScriptBlock = {
    try {
