<#
.SYNOPSIS
    CIS Audit Script for 17.7.3 - Ensure 'Audit Authorization Policy Change' is set to include 'Success'
.DESCRIPTION
    This script audits the configuration of Audit Authorization Policy Change settings using auditpol.exe.
    It checks if the subcategory is properly configured to include success auditing.
.NOTES
    CIS ID: 17.7.3
    Profile: L1
    File Name: 17.7.3-audit-authorization-policy-change.ps1
...
$auditScriptBlock = {
    try {
        ...
        $auditResult = auditpol /get /subcategory:"{0cce9231-69ae-11d9-bed3-505054503030}"
        
        # Parse the output to extract the current setting
        $currentSetting = "Unknown"
        foreach ($line in $auditResult) {
            if ($line -match "Authorization Policy Change" -and $line -match "Success and Failure|Success|Failure|No Auditing") {
        ...
        return @{
            ...
            Source = "auditpol.exe"
            Details = "Subcategory GUID: {0cce9231-69ae-11d9-bed3-505054503030}"
