<#
.SYNOPSIS
    CIS Audit Script for 17.5.2 - Ensure 'Audit Group Membership' is set to include 'Success'
.DESCRIPTION
    This script audits the configuration of Audit Group Membership settings using auditpol.exe.
    It checks if the subcategory is properly configured to include success auditing.
.NOTES
    CIS ID: 17.5.2
    Profile: L1
    File Name: 17.5.2-audit-group-membership.ps1
...
$auditScriptBlock = {
    try {
        ...
        return @{
            ...
            Source = "auditpol.exe"
            Details = "Subcategory GUID: {0cce9249-69ae-11d9-bed3-505054503030}"
