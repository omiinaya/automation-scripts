...
# Initialize result object
    ...
    if ($VerboseOutput) {
        Write-SectionHeader -Title "Password Policy Remediation: Password Complexity Requirements"
    }
    ...
    if ($isDomainMember) {
        if ($VerboseOutput) {
            ...
            Write-Host "4. Set 'Password must meet complexity requirements' to Enabled" -ForegroundColor White
...
"@
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                $scriptResult = [PSCustomObject]@{
                    Status = "Remediated"
                    Message = "Password complexity requirements successfully enabled"
                    PreviousValue = "Unknown"
                    NewValue = "Enabled"
                    IsCompliant = $true
