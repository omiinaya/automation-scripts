# CIS Remediation Script for 2.3.1.3: Accounts: Rename administrator account
...
.DESCRIPTION
    This script remediates the compliance status of CIS control 2.3.1.3 which ensures
    that the built-in Administrator account is renamed to something other than
    the default "Administrator" name.

.PARAMETER NewName
    The new name to assign to the Administrator account.

.PARAMETER Force
    If specified, will apply remediation without prompting for confirmation.

.EXAMPLE
    .\2.3.1.3-remediate-rename-administrator-account.ps1 -NewName "SysAdmin01"

.EXAMPLE
    .\2.3.1.3-remediate-rename-administrator-account.ps1 -NewName "SysAdmin01" -Force
...
.NOTES
    ...
    Required Value: Any value other than "Administrator"
...
if ($NewName -eq "Administrator") {
    Write-Error "The new name cannot be 'Administrator'. Please choose a different name."
    exit 1
}
...
Write-Host ""
Write-Host "Security Policy: Rename Administrator account" -ForegroundColor Cyan
Write-Host "  Set to: $NewName" -ForegroundColor White
Write-Host "  Purpose: Rename the built-in Administrator account for security" -ForegroundColor Gray
Write-Host ""
...
Revision=1
[Registry Values]
MACHINE\SYSTEM\CurrentControlSet\Control\Lsa\NewAdministratorName=$NewName
"@
    ...
    $appliedValue = ($verifyContent | Where-Object { $_ -like "NewAdministratorName*" } | ForEach-Object { ($_ -split "=")[1].Trim() })
    ...
    if ($appliedValue -eq $NewName) {
        ...
        Write-Host "  SUCCESS: Administrator account renamed to '$NewName'" -ForegroundColor Green
    } else {
        $result = @{
            Description = "Administrator account name"
            Success = $false
        ...
        Write-Host "  FAILED: Could not verify Administrator account rename" -ForegroundColor Red
    ...
    Write-Host "  ERROR: Failed to rename Administrator account" -ForegroundColor Red
...
if ($OverallSuccess) {
    ...
    Write-Host "The Administrator account has been renamed to '$NewName' according to CIS benchmark requirements." -ForegroundColor Green
    Write-Host ""
