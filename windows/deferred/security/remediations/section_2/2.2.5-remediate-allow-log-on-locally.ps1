# Remediation: Allow log on locally setting on Windows
...
try {
    if ($VerboseOutput) {
        Write-SectionHeader -Title "User Rights Assignment Remediation: Allow log on locally"
    }
...
"@
    ...
    $result = Invoke-CISRemediation -CIS_ID "2.2.5" -RemediationType "SecurityPolicy" -SecurityPolicyTemplate $templateContent -SettingName "SeInteractiveLogonRight" -VerboseOutput:$VerboseOutput
