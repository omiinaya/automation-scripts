<#
.SYNOPSIS
    CIS Remediation Script for 2.3.9.3 - Ensure 'Microsoft network server: Digitally sign communications (if client agrees)' is set to 'Enabled'
.DESCRIPTION
    This script remediates the setting that determines whether the SMB server will negotiate SMB packet signing with clients that request it.
...
.NOTES
    File Name      : 2.3.9.3-remediate-microsoft-network-server-digitally-sign-communications-if-client-agrees.ps1
    CIS ID         : 2.3.9.3
...
if (-not (Test-AdminRights)) {
    Write-Host "Administrator privileges required for Microsoft network server digitally sign communications (if client agrees) remediation." -ForegroundColor Yellow
...
# Function to configure Microsoft network server digitally sign communications (if client agrees)
function Configure-MicrosoftNetworkServerDigitallySignCommunicationsIfClientAgrees {
    <#
    .SYNOPSIS
        Configures the Microsoft network server digitally sign communications (if client agrees) setting
    .DESCRIPTION
        Sets registry value to negotiate SMB packet signing with clients that request it
    .OUTPUTS
        PSCustomObject
    #>
    ...
    try {
        ...
        Write-Error "Failed to configure Microsoft network server digitally sign communications (if client agrees): $_"
        return [PSCustomObject]@{
            PreviousValue = "Error"
            NewValue = "Error"
            Success = $false
            Message = "Failed to configure Microsoft network server digitally sign communications (if client agrees): $_"
            RemediationApplied = $false
        }
    }
}

# Function to configure Group Policy for Microsoft network server digitally sign communications (if client agrees)
function Configure-MicrosoftNetworkServerDigitallySignCommunicationsIfClientAgreesGroupPolicy {
    <#
    .SYNOPSIS
        Configures Group Policy for Microsoft network server digitally sign communications (if client agrees)
    .DESCRIPTION
        Sets registry value to negotiate SMB packet signing with clients that request it via Group Policy
    .OUTPUTS
        PSCustomObject
    #>
    ...
    try {
        ...
        if ($policyValue -eq 1) {
            ...
            Write-Host "SMB packet signing will be negotiated with clients that request it via Group Policy" -ForegroundColor Green
            return [PSCustomObject]@{
                Success = $true
...
function Invoke-MicrosoftNetworkServerDigitallySignCommunicationsIfClientAgreesRemediation {
    <#
    .SYNOPSIS
        Main function to execute Microsoft network server digitally sign communications (if client agrees) remediation
    .DESCRIPTION
        Performs comprehensive remediation of the Microsoft network server digitally sign communications (if client agrees) setting
    .OUTPUTS
        PSCustomObject
    #>
    ...
    try {
        ...
        $configureResult = Configure-MicrosoftNetworkServerDigitallySignCommunicationsIfClientAgrees
        
        # Configure Group Policy setting
        $groupPolicyResult = Configure-MicrosoftNetworkServerDigitallySignCommunicationsIfClientAgreesGroupPolicy
        ...
        $result = New-CISRemediationResult -CIS_ID "2.3.9.3" -Title "Ensure 'Microsoft network server: Digitally sign communications (if client agrees)' is set to 'Enabled'" -PreviousValue $configureResult.PreviousValue -NewValue $configureResult.NewValue -Status $(if ($overallSuccess) { "Remediated" } else { "PartiallyRemediated" }) -Message "Microsoft network server digitally sign communications (if client agrees) remediation completed" -IsCompliant $overallSuccess -RequiresManualAction $false -Source "Registry"
        ...
        Write-Error "Microsoft network server digitally sign communications (if client agrees) remediation failed: $_"
        return New-CISRemediationResult -CIS_ID "2.3.9.3" -Title "Ensure 'Microsoft network server: Digitally sign communications (if client agrees)' is set to 'Enabled'" -PreviousValue "Error" -NewValue "Error" -Status "Error" -Message "Remediation failed: $_" -IsCompliant $false -RequiresManualAction $true -ErrorMessage $_
    }
}
...
if ($MyInvocation.InvocationName -ne ".") {
    try {
        $remediationResult = Invoke-MicrosoftNetworkServerDigitallySignCommunicationsIfClientAgreesRemediation
...
# Export functions
Export-ModuleMember -Function Configure-MicrosoftNetworkServerDigitallySignCommunicationsIfClientAgrees, Configure-MicrosoftNetworkServerDigitallySignCommunicationsIfClientAgreesGroupPolicy, Invoke-MicrosoftNetworkServerDigitallySignCommunicationsIfClientAgreesRemediation