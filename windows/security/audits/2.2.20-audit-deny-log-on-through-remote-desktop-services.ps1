# Audit: Deny log on through Remote Desktop Services setting on Windows
...
try {
    if ($VerboseOutput) {
        Write-SectionHeader -Title "User Rights Assignment Audit: Deny log on through Remote Desktop Services"
    }
    
    # Define custom audit script block
    $auditScriptBlock = {
        # Get the current user rights assignment
        $currentRights = Get-UserRightsAssignment -Privilege "SeDenyRemoteInteractiveLogonRight"
        ...
        @{
            ...
            Description = "Deny log on through Remote Desktop Services should include Guests group"
        }
    }
    
    # Invoke audit using CISFramework
    $result = Invoke-CISAudit -CIS_ID "2.2.20" -AuditScriptBlock $auditScriptBlock -VerboseOutput:$VerboseOutput
