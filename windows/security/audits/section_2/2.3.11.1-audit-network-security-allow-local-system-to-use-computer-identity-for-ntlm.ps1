<#
.SYNOPSIS
    Audit script for CIS ID 2.3.11.1: Network security: Allow Local System to use computer identity for NTLM

.DESCRIPTION
    This script audits the configuration of 'Network security: Allow Local System to use computer identity for NTLM' 
    to ensure it is set to 'Enabled' as recommended by CIS Benchmark.

.PARAMETER None
    This script does not accept parameters.

.EXAMPLE
    .\2.3.11.1-audit-network-security-allow-local-system-to-use-computer-identity-for-ntlm.ps1

.OUTPUTS
    Returns a custom object with audit results

.NOTES
    CIS ID: 2.3.11.1
    Title: Ensure 'Network security: Allow Local System to use computer identity for NTLM' is set to 'Enabled'
    Profile: L1
    Registry Path: HKLM\SYSTEM\CurrentControlSet\Control\Lsa
    Registry Value: UseMachineId
    Recommended Value: 1 (Enabled)
#>

# Import the CIS Framework module
Import-Module -Name "$PSScriptRoot\..\..\modules\CISFramework.psm1" -Force

# Define audit parameters
$CisId = "2.3.11.1"
$RegistryPath = "HKLM:\SYSTEM\CurrentControlSet\Control\Lsa"
$RegistryValueName = "UseMachineId"

# Execute the audit using Invoke-CISAudit
$AuditResult = Invoke-CISAudit -CIS_ID $CisId -AuditType "Registry" -RegistryPath $RegistryPath -RegistryValueName $RegistryValueName -Section "2" -VerboseOutput

# Output the result
return $AuditResult