<#
.SYNOPSIS
    CIS Audit Script for 17.5.6 - Ensure 'Audit Special Logon' is set to include 'Success'
.DESCRIPTION
    This script audits the configuration of Audit Special Logon settings using auditpol.exe.
    It checks if the subcategory is properly configured to include success auditing.
.NOTES
    CIS ID: 17.5.6
    Profile: L1
    File Name: 17.5.6-audit-special-logon.ps1
...
$auditScriptBlock = {
    try {
        ...
        $auditResult = auditpol /get /subcategory:"{0cce921b-69ae-11d9-bed3-505054503030}"
        
        # Parse the output to extract the current setting
        $currentSetting = "Unknown"
        foreach ($line in $auditResult) {
            if ($line -match "Special Logon" -and $line -match "Success and Failure|Success|Failure|No Auditing") {
        ...
        return @{
            ...
            Source = "auditpol.exe"
            Details = "Subcategory GUID: {0cce921b-69ae-11d9-bed3-505054503030}"
