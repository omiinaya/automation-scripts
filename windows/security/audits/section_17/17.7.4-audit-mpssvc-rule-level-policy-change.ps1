<#
.SYNOPSIS
    CIS Audit Script for 17.7.4 - Ensure 'Audit MPSSVC Rule-Level Policy Change' is set to 'Success and Failure'
.DESCRIPTION
    This script audits the configuration of Audit MPSSVC Rule-Level Policy Change settings using auditpol.exe.
    It checks if the subcategory is properly configured to include both success and failure auditing.
.NOTES
    CIS ID: 17.7.4
    Profile: L1
    File Name: 17.7.4-audit-mpssvc-rule-level-policy-change.ps1
...
$auditScriptBlock = {
    try {
        ...
        $auditResult = auditpol /get /subcategory:"{0cce9232-69ae-11d9-bed3-505054503030}"
        
        # Parse the output to extract the current setting
        $currentSetting = "Unknown"
        foreach ($line in $auditResult) {
            if ($line -match "MPSSVC Rule-Level Policy Change" -and $line -match "Success and Failure|Success|Failure|No Auditing") {
        ...
        return @{
            ...
            Source = "auditpol.exe"
            Details = "Subcategory GUID: {0cce9232-69ae-11d9-bed3-505054503030}"
