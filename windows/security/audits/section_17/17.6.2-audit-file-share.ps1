<#
.SYNOPSIS
    CIS Audit Script for 17.6.2 - Ensure 'Audit File Share' is set to 'Success and Failure'
.DESCRIPTION
    This script audits the configuration of Audit File Share settings using auditpol.exe.
    It checks if the subcategory is properly configured to include both success and failure auditing.
.NOTES
    CIS ID: 17.6.2
    Profile: L1
    File Name: 17.6.2-audit-file-share.ps1
...
$auditScriptBlock = {
    try {
        ...
        $auditResult = auditpol /get /subcategory:"{0cce9224-69ae-11d9-bed3-505054503030}"
        
        # Parse the output to extract the current setting
        $currentSetting = "Unknown"
        foreach ($line in $auditResult) {
            if ($line -match "File Share" -and $line -match "Success and Failure|Success|Failure|No Auditing") {
        ...
        return @{
            ...
            Source = "auditpol.exe"
            Details = "Subcategory GUID: {0cce9224-69ae-11d9-bed3-505054503030}"
