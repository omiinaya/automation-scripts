<#
.SYNOPSIS
    CIS Audit Script for 17.7.1 - Ensure 'Audit Audit Policy Change' is set to include 'Success'
.DESCRIPTION
    This script audits the configuration of Audit Audit Policy Change settings using auditpol.exe.
    It checks if the subcategory is properly configured to include success auditing.
.NOTES
    CIS ID: 17.7.1
    Profile: L1
    File Name: 17.7.1-audit-audit-policy-change.ps1
...
$auditScriptBlock = {
    try {
        ...
        $auditResult = auditpol /get /subcategory:"{0cce922f-69ae-11d9-bed3-505054503030}"
        
        # Parse the output to extract the current setting
        $currentSetting = "Unknown"
        foreach ($line in $auditResult) {
            if ($line -match "Audit Policy Change" -and $line -match "Success and Failure|Success|Failure|No Auditing") {
        ...
        return @{
            ...
            Source = "auditpol.exe"
            Details = "Subcategory GUID: {0cce922f-69ae-11d9-bed3-505054503030}"
