...
.NOTES
    CIS ID: 19.7.8.1
    Profile: L1
    File Name: 19.7.8.1-audit-configure-windows-spotlight-on-lock-screen.ps1
...
# Audit Windows Spotlight on lock screen setting
...
try {
    ...
    if (Test-Path $registryPath) {
        ...
        if ($currentValue -and $currentValue.$valueName -eq 2) {
            $result = New-CISResultObject -CIS_ID $cisId -Title $recommendation.title -CurrentValue "Disabled (2)" -RecommendedValue "Disabled" -ComplianceStatus "Compliant" -Source "Registry" -Details "Windows Spotlight on lock screen is disabled" -Profile $recommendation.profile
        } else {
            $result = New-CISResultObject -CIS_ID $cisId -Title $recommendation.title -CurrentValue "Enabled or Not Configured" -RecommendedValue "Disabled" -ComplianceStatus "Non-Compliant" -Source "Registry" -Details "Windows Spotlight on lock screen is enabled" -Profile $recommendation.profile
        }
    } else {
