<#
.SYNOPSIS
    CIS Audit Script for 2.3.9.2 - Ensure 'Microsoft network server: Digitally sign communications (always)' is set to 'Enabled'
.DESCRIPTION
    This script audits the setting that determines whether packet signing is required by the SMB server component.
...
.NOTES
    File Name      : 2.3.9.2-audit-microsoft-network-server-digitally-sign-communications-always.ps1
    CIS ID         : 2.3.9.2
...
# Function to audit Microsoft network server digitally sign communications (always)
function Audit-MicrosoftNetworkServerDigitallySignCommunicationsAlways {
    <#
    .SYNOPSIS
        Audits the Microsoft network server digitally sign communications (always) setting
    .DESCRIPTION
        Checks if packet signing is required by the SMB server component as recommended by CIS benchmarks
    .OUTPUTS
        PSCustomObject
    #>
    ...
    try {
        ...
        Write-Error "Failed to audit Microsoft network server digitally sign communications (always) setting: $_"
        
        # Return error result
        return New-CISResultObject -CIS_ID "2.3.9.2" -Title "Ensure 'Microsoft network server: Digitally sign communications (always)' is set to 'Enabled'" -CurrentValue "Error" -RecommendedValue "Enabled (1)" -ComplianceStatus "Error" -Source "Registry" -ErrorMessage "Audit failed: $_" -Profile "L1"
    }
}

# Function to check Group Policy setting
function Test-MicrosoftNetworkServerDigitallySignCommunicationsAlwaysGroupPolicy {
    <#
    .SYNOPSIS
        Checks Group Policy setting for Microsoft network server digitally sign communications (always)
    .DESCRIPTION
        Verifies if Group Policy is configured to require packet signing by the SMB server component
    .OUTPUTS
        PSCustomObject
    #>
    ...
    try {
        ...
        if (Test-RegistryKey -KeyPath $registryPath) {
            ...
            if ($policyValue -eq "Not Configured") {
                ...
                $details = "Group Policy not configured for Microsoft network server digitally sign communications (always)"
...
function Invoke-MicrosoftNetworkServerDigitallySignCommunicationsAlwaysAudit {
    <#
    .SYNOPSIS
        Main function to execute Microsoft network server digitally sign communications (always) audit
    .DESCRIPTION
        Performs comprehensive audit of the Microsoft network server digitally sign communications (always) setting
    .OUTPUTS
        PSCustomObject
    #>
    ...
    try {
        ...
        # Perform main audit
        $mainResult = Audit-MicrosoftNetworkServerDigitallySignCommunicationsAlways
        
        # Check Group Policy setting
        $groupPolicyResult = Test-MicrosoftNetworkServerDigitallySignCommunicationsAlwaysGroupPolicy
        
        # Combine results
        if ($groupPolicyResult.IsCompliant) {
            Write-Host ""
            Write-Host "Group Policy is properly configured to require packet signing by the SMB server component." -ForegroundColor Green
        } else {
            Write-Host ""
            Write-Host "Group Policy is not configured to require packet signing by the SMB server component." -ForegroundColor Yellow
        }
        
        return $mainResult
    }
    catch {
        Write-Error "Microsoft network server digitally sign communications (always) audit failed: $_"
        return New-CISResultObject -CIS_ID "2.3.9.2" -Title "Ensure 'Microsoft network server: Digitally sign communications (always)' is set to 'Enabled'" -CurrentValue "Error" -RecommendedValue "Enabled (1)" -ComplianceStatus "Error" -Source "Registry" -ErrorMessage "Audit failed: $_" -Profile "L1"
    }
}
...
if ($MyInvocation.InvocationName -ne ".") {
    try {
        $auditResult = Invoke-MicrosoftNetworkServerDigitallySignCommunicationsAlwaysAudit
...
# Export functions
Export-ModuleMember -Function Audit-MicrosoftNetworkServerDigitallySignCommunicationsAlways, Test-MicrosoftNetworkServerDigitallySignCommunicationsAlwaysGroupPolicy, Invoke-MicrosoftNetworkServerDigitallySignCommunicationsAlwaysAudit