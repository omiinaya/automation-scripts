<#
.SYNOPSIS
    CIS Audit Script for 18.5.11 - Ensure 'MSS: (TcpMaxDataRetransmissions IPv6) How many times unacknowledged data is retransmitted' is set to 'Enabled: 3'
.DESCRIPTION
    This script audits the MSS (TcpMaxDataRetransmissions IPv6) registry setting to ensure TCP retransmits unacknowledged data no more than 3 times.
    The setting checks HKLM\SYSTEM\CurrentControlSet\Services\TCPIP6\Parameters:TcpMaxDataRetransmissions
.NOTES
    CIS ID: 18.5.11
    Profile: L2
    File Name: 18.5.11-audit-mss-tcpmaxdataretransmissions-ipv6.ps1
...
# Custom audit script block for MSS TcpMaxDataRetransmissions IPv6 checking
$auditScriptBlock = {
    try {
