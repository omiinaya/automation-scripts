<#
.SYNOPSIS
    CIS Audit Script for 18.5.2 - Ensure 'MSS: (DisableIPSourceRouting IPv6) IP source routing protection level' is set to 'Enabled: Highest protection, source routing is completely disabled'
.DESCRIPTION
    This script audits the MSS (DisableIPSourceRouting IPv6) registry setting to ensure IP source routing protection is at highest level.
    The setting checks HKLM\SYSTEM\CurrentControlSet\Services\Tcpip6\Parameters:DisableIPSourceRouting
.NOTES
    CIS ID: 18.5.2
    Profile: L1
    File Name: 18.5.2-audit-mss-disableipsourcerouting-ipv6.ps1
...
# Custom audit script block for MSS DisableIPSourceRouting IPv6 checking
$auditScriptBlock = {
    try {
