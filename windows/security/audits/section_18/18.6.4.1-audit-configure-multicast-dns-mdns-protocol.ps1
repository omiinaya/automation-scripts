<#
.SYNOPSIS
    CIS Audit Script for 18.6.4.1 - Ensure 'Configure multicast DNS (mDNS) protocol' is set to 'Disabled'
.DESCRIPTION
    This script audits the multicast DNS (mDNS) protocol setting to ensure it is disabled.
    The setting checks HKLM\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient:EnableMDNS
.NOTES
    CIS ID: 18.6.4.1
    Profile: L1
    File Name: 18.6.4.1-audit-configure-multicast-dns-mdns-protocol.ps1
...
$registryValueName = "EnableMDNS"

# Custom audit script block for mDNS protocol checking
$auditScriptBlock = {
    try {
        ...
        if (Test-Path $registryPath) {
            ...
            if ($currentValue) {
                ...
                $currentSetting = if ($value -eq 0) { "Disabled" } else { "Enabled" }
            } else {
        ...
        # Determine compliance (must be Disabled - value 0)
        $isCompliant = ($value -eq 0)
