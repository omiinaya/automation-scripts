<#
.SYNOPSIS
    CIS Remediation Script for 17.2.2 - Ensure 'Audit Security Group Management' is set to include 'Success'
.DESCRIPTION
    This script remediates the configuration of Audit Security Group Management settings using auditpol.exe.
    It configures the subcategory to include success auditing.
.NOTES
    CIS ID: 17.2.2
    Profile: L1
    File Name: 17.2.2-remediate-security-group-management.ps1
...
$remediationScriptBlock = {
    try {
        ...
        $auditResult = auditpol /get /subcategory:"{0cce9237-69ae-11d9-bed3-505054503030}"
        $previousSetting = "Unknown"
        foreach ($line in $auditResult) {
            if ($line -match "Security Group Management" -and $line -match "Success and Failure|Success|Failure|No Auditing") {
        ...
        $setResult = auditpol /set /subcategory:"{0cce9237-69ae-11d9-bed3-505054503030}" /success:enable
        
        if ($LASTEXITCODE -eq 0) {
            # Verify the change
            Start-Sleep -Seconds 2
            $verifyResult = auditpol /get /subcategory:"{0cce9237-69ae-11d9-bed3-505054503030}"
            $newSetting = "Unknown"
