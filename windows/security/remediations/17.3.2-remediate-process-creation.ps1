<#
.SYNOPSIS
    CIS Remediation Script for 17.3.2 - Ensure 'Audit Process Creation' is set to include 'Success'
.DESCRIPTION
    This script remediates the configuration of Audit Process Creation settings using auditpol.exe.
    It configures the subcategory to include success auditing.
.NOTES
    CIS ID: 17.3.2
    Profile: L1
    File Name: 17.3.2-remediate-process-creation.ps1
...
$remediationScriptBlock = {
    try {
        ...
        $auditResult = auditpol /get /subcategory:"{0cce922b-69ae-11d9-bed3-505054503030}"
        $previousSetting = "Unknown"
        foreach ($line in $auditResult) {
            if ($line -match "Process Creation" -and $line -match "Success and Failure|Success|Failure|No Auditing") {
        ...
        $setResult = auditpol /set /subcategory:"{0cce922b-69ae-11d9-bed3-505054503030}" /success:enable
        
        if ($LASTEXITCODE -eq 0) {
            # Verify the change
            Start-Sleep -Seconds 2
            $verifyResult = auditpol /get /subcategory:"{0cce922b-69ae-11d9-bed3-505054503030}"
            $newSetting = "Unknown"
