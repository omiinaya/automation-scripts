...
try {
    ...
    if ($minimumPasswordAge -ge 1) {
        if ($VerboseOutput) {
            Write-StatusMessage -Message "COMPLIANT: Minimum password age setting already meets CIS benchmark" -Type Success
            Write-Host ""
        ...
        $scriptResult = [PSCustomObject]@{
            Status = "Compliant"
            Message = "Minimum password age setting already meets CIS benchmark"
            PreviousValue = $minimumPasswordAge
            NewValue = $minimumPasswordAge
            IsCompliant = $true
        ...
        if ($VerboseOutput) {
            ...
            Write-Host "The minimum password age setting needs to be changed from $minimumPasswordAge to 1 (or more day(s))." -ForegroundColor White
            Write-Host ""
        }
        ...
        if ($isDomainMember) {
            if ($VerboseOutput) {
                ...
                Write-Host "4. Set 'Minimum password age' to 1 or more day(s)" -ForegroundColor White
...
"@
                ...
                if ($LASTEXITCODE -eq 0) {
                    ...
                    $verifyLine = $verifyContent | Where-Object { $_ -like "MinimumPasswordAge*" }
                    
                    if ($verifyLine) {
                        $newValue = [int]($verifyLine -split "=")[1].Trim()
                        if ($newValue -ge 1) {
                            if ($VerboseOutput) {
                                Write-StatusMessage -Message "Remediation verified: Minimum password age now set to $newValue" -Type Success
                            }
                            $scriptResult = [PSCustomObject]@{
                                Status = "Remediated"
                                Message = "Minimum password age setting successfully updated to $newValue"
                                PreviousValue = $minimumPasswordAge
                                NewValue = $newValue
                                IsCompliant = $true
                                RequiresManualAction = $false
                            ...
                            $scriptResult = [PSCustomObject]@{
                                ...
                                Message = "Minimum password age setting may not have been applied correctly (value: $newValue)"
                                PreviousValue = $minimumPasswordAge
                                NewValue = $newValue
                                IsCompliant = $false
                                RequiresManualAction = $false
                        ...
                        $scriptResult = [PSCustomObject]@{
                            Status = "PartiallyRemediated"
                            Message = "Unable to verify minimum password age setting after remediation"
                            PreviousValue = $minimumPasswordAge
                            NewValue = 1
                            IsCompliant = $false
                            RequiresManualAction = $false
                ...
                if ($VerboseOutput) {
                    ...
                    Write-Host "Please manually set the minimum password age policy:" -ForegroundColor White
                    Write-Host "1. Open Local Security Policy (secpol.msc)" -ForegroundColor White
                    Write-Host "2. Navigate to: Account Policies\Password Policy" -ForegroundColor White
                    Write-Host "3. Set 'Minimum password age' to 1 or more day(s)" -ForegroundColor White
                    Write-Host "4. Click Apply and OK" -ForegroundColor White
        ...
        if ($VerboseOutput) {
            ...
            $remediationData = @(
                [PSCustomObject]@{
                    ...
                    OriginalValue = "$minimumPasswordAge day(s)"
                    NewValue = "1 day(s)"
            ...
            if ($isDomainMember) {
                ...
                Write-Host "• Users cannot change passwords immediately after setting them" -ForegroundColor White
