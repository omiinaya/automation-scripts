<#
.SYNOPSIS
    Remediates CIS control 19.5.1.1 - Turn off toast notifications on the lock screen
.DESCRIPTION
    This script remediates the toast notifications on lock screen setting to comply with CIS recommendations.
.NOTES
    CIS ID: 19.5.1.1
    Profile: L1
    File Name: 19.5.1.1-remediate-turn-off-toast-notifications-on-lock-screen.ps1
...
# Remediate toast notifications on lock screen setting
...
try {
    ...
    if ($previousValue -ne "Already Compliant") {
        ...
        if ($newValue.$valueName -eq $expectedValue) {
            $result = New-CISRemediationResult -CIS_ID $cisId -Title $recommendation.title -PreviousValue $previousValue -NewValue $expectedValue -Status "Remediated" -Message "Toast notifications on lock screen successfully disabled" -IsCompliant $true -RequiresManualAction $false -Source "Registry"
        } else {
