<#
.SYNOPSIS
    CIS Remediation Script for 2.3.8.1 - Ensure 'Microsoft network client: Digitally sign communications (always)' is set to 'Enabled'
.DESCRIPTION
    This script remediates the setting that determines whether packet signing is required by the SMB client component.
...
.NOTES
    File Name      : 2.3.8.1-remediate-microsoft-network-client-digitally-sign-communications-always.ps1
    CIS ID         : 2.3.8.1
...
if (-not (Test-AdminRights)) {
    Write-Host "Administrator privileges required for Microsoft network client digitally sign communications remediation." -ForegroundColor Yellow
...
# Function to configure Microsoft network client digitally sign communications (always)
function Configure-MicrosoftNetworkClientDigitallySignCommunicationsAlways {
    <#
    .SYNOPSIS
        Configures the Microsoft network client digitally sign communications (always) setting
    .DESCRIPTION
        Sets registry value to require SMB packet signing for all communications
    .OUTPUTS
        PSCustomObject
    #>
    ...
    try {
        Write-Host ""
        Write-Host "=== CIS Remediation: 2.3.8.1 - Microsoft Network Client: Digitally Sign Communications (Always) ===" -ForegroundColor Cyan
        Write-Host "Configuring Microsoft network client digitally sign communications (always) setting..." -ForegroundColor White
        ...
        if (Test-RegistryKey -KeyPath $registryPath) {
            ...
            if ($currentValue -eq "Not Set") {
                # Default is "Disabled" when not set
            ...
            if ($newValue -eq 1) {
                Write-Host "Microsoft network client digitally sign communications (always) successfully configured to $newStatus" -ForegroundColor Green
                Write-Host "SMB packet signing will now be required for all communications" -ForegroundColor Green
                return [PSCustomObject]@{
                    PreviousValue = $previousStatus
                    NewValue = $newStatus
                    Success = $true
                    Message = "Microsoft network client digitally sign communications (always) successfully configured to $newStatus"
                    RemediationApplied = $true
                }
            } else {
                Write-Host "Failed to configure Microsoft network client digitally sign communications (always) within CIS recommendation" -ForegroundColor Red
                return [PSCustomObject]@{
                    PreviousValue = $previousStatus
                    NewValue = $newStatus
                    Success = $false
                    Message = "Failed to configure Microsoft network client digitally sign communications (always) within CIS recommendation"
                    RemediationApplied = $true
                }
            }
        } else {
        ...
        Write-Error "Failed to configure Microsoft network client digitally sign communications (always): $_"
        return [PSCustomObject]@{
            PreviousValue = "Error"
            NewValue = "Error"
            Success = $false
            Message = "Failed to configure Microsoft network client digitally sign communications (always): $_"
            RemediationApplied = $false
        }
    }
}

# Function to configure Group Policy for Microsoft network client digitally sign communications (always)
function Configure-MicrosoftNetworkClientDigitallySignCommunicationsAlwaysGroupPolicy {
    <#
    .SYNOPSIS
        Configures Group Policy for Microsoft network client digitally sign communications (always)
    .DESCRIPTION
        Sets registry value to require SMB packet signing for all communications via Group Policy
    .OUTPUTS
        PSCustomObject
    #>
    ...
    try {
        ...
        if ($policyValue -eq 1) {
            ...
            Write-Host "SMB packet signing will be required for all communications via Group Policy" -ForegroundColor Green
            return [PSCustomObject]@{
                Success = $true
...
function Invoke-MicrosoftNetworkClientDigitallySignCommunicationsAlwaysRemediation {
    <#
    .SYNOPSIS
        Main function to execute Microsoft network client digitally sign communications (always) remediation
    .DESCRIPTION
        Performs comprehensive remediation of the Microsoft network client digitally sign communications (always) setting
    .OUTPUTS
        PSCustomObject
    #>
    ...
    try {
        ...
        $configureResult = Configure-MicrosoftNetworkClientDigitallySignCommunicationsAlways
        
        # Configure Group Policy setting
        $groupPolicyResult = Configure-MicrosoftNetworkClientDigitallySignCommunicationsAlwaysGroupPolicy
        ...
        $result = New-CISRemediationResult -CIS_ID "2.3.8.1" -Title "Ensure 'Microsoft network client: Digitally sign communications (always)' is set to 'Enabled'" -PreviousValue $configureResult.PreviousValue -NewValue $configureResult.NewValue -Status $(if ($overallSuccess) { "Remediated" } else { "PartiallyRemediated" }) -Message "Microsoft network client digitally sign communications (always) remediation completed" -IsCompliant $overallSuccess -RequiresManualAction $false -Source "Registry"
        ...
        Write-Error "Microsoft network client digitally sign communications (always) remediation failed: $_"
        return New-CISRemediationResult -CIS_ID "2.3.8.1" -Title "Ensure 'Microsoft network client: Digitally sign communications (always)' is set to 'Enabled'" -PreviousValue "Error" -NewValue "Error" -Status "Error" -Message "Remediation failed: $_" -IsCompliant $false -RequiresManualAction $true -ErrorMessage $_
    }
}
...
if ($MyInvocation.InvocationName -ne ".") {
    try {
        $remediationResult = Invoke-MicrosoftNetworkClientDigitallySignCommunicationsAlwaysRemediation
...
# Export functions
Export-ModuleMember -Function Configure-MicrosoftNetworkClientDigitallySignCommunicationsAlways, Configure-MicrosoftNetworkClientDigitallySignCommunicationsAlwaysGroupPolicy, Invoke-MicrosoftNetworkClientDigitallySignCommunicationsAlwaysRemediation