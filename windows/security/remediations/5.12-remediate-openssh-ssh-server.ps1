# Remediation: OpenSSH SSH Server (sshd) setting on Windows
...
try {
    if ($VerboseOutput) {
        Write-SectionHeader -Title "Service Remediation: OpenSSH SSH Server (sshd)"
    }
    
    # Use Invoke-CISRemediation with Custom remediation type for service configuration
    $remediationResult = Invoke-CISRemediation -CIS_ID "5.12" -RemediationType "Custom" -VerboseOutput:$VerboseOutput -Section "5" -CustomScriptBlock {
        try {
            # Check if service exists
            if (Test-ServiceExists -ServiceName "sshd") {
                ...
                # Verify the change
                $updatedService = Get-Service -Name "sshd"
            ...
            throw "Failed to remediate sshd service: $($_.Exception.Message)"
        }
    }
