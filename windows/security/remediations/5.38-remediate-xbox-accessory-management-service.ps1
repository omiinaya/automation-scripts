# Remediation: Xbox Accessory Management Service (XboxGipSvc) setting on Windows
...
try {
    if ($VerboseOutput) {
        Write-SectionHeader -Title "Service Remediation: Xbox Accessory Management Service (XboxGipSvc)"
    }
    
    # Use Invoke-CISRemediation with Custom remediation type for service configuration
    $remediationResult = Invoke-CISRemediation -CIS_ID "5.38" -RemediationType "Custom" -VerboseOutput:$VerboseOutput -Section "5" -CustomScriptBlock {
        try {
            # Check if service exists
            if (Test-ServiceExists -ServiceName "XboxGipSvc") {
                # Get current service status
                $service = Get-Service -Name "XboxGipSvc"
                ...
                # Verify the change
                $updatedService = Get-Service -Name "XboxGipSvc"
            ...
            throw "Failed to remediate XboxGipSvc: $($_.Exception.Message)"
        }
    }
