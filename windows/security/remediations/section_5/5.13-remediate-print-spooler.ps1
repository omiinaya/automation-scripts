...
try {
    if ($VerboseOutput) {
        Write-SectionHeader -Title "Service Remediation: Print Spooler (Spooler)"
    }
    
    # Use Invoke-CISRemediation with Custom remediation type for service configuration
    $remediationResult = Invoke-CISRemediation -CIS_ID "5.13" -RemediationType "Custom" -VerboseOutput:$VerboseOutput -Section "5" -CustomScriptBlock {
        try {
            ...
            throw "Failed to remediate Spooler service: $($_.Exception.Message)"
        }
    }
