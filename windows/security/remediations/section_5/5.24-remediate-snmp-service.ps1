# Remediation: SNMP Service (SNMP) setting on Windows
...
try {
    if ($VerboseOutput) {
        Write-SectionHeader -Title "Service Remediation: SNMP Service (SNMP)"
    }
    
    # Use Invoke-CISRemediation with Custom remediation type for service configuration
    $remediationResult = Invoke-CISRemediation -CIS_ID "5.24" -RemediationType "Custom" -VerboseOutput:$VerboseOutput -Section "5" -CustomScriptBlock {
        try {
            # Check if service exists
            if (Test-ServiceExists -ServiceName "SNMP") {
                ...
                # Verify the change
                $updatedService = Get-Service -Name "SNMP"
            ...
            throw "Failed to remediate SNMP: $($_.Exception.Message)"
        }
    }
