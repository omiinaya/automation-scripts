...
try {
    ...
    if (-not $service) {
        Write-StatusMessage -Message "SysMain service (formerly Superfetch/Prefetch) not found on this system" -Type Warning
    ...
    Write-StatusMessage -Message "Current SysMain service status: $($service.Status)" -Type Info
    ...
    if ($service.StartType -eq "Disabled") {
        ...
        Write-StatusMessage -Message "SysMain service enabled and started" -Type Success
        Write-StatusMessage -Message "Prefetch/Superfetch functionality is now active" -Type Warning
    } else {
        ...
        Write-StatusMessage -Message "SysMain service stopped and disabled" -Type Success
        Write-StatusMessage -Message "Prefetch/Superfetch functionality is now disabled" -Type Warning
        Write-StatusMessage -Message "Note: Disabling SysMain may improve gaming performance by reducing disk activity" -Type Info
    }
    
    # Verify the new startup type
    Start-Sleep -Seconds 2
    $newService = Get-Service -Name "SysMain"
    Write-StatusMessage -Message "New SysMain service startup type: $($newService.StartType)" -Type Info
    ...
    Wait-OnError -ErrorMessage "Failed to toggle SysMain service startup type: $($_.Exception.Message)"
}