...
# Controls whether windows have drop shadows
...
try {
    # Window shadows are partially controlled by ListviewShadow
    # This affects shadows under windows and some UI elements
    ...
    # Toggle the setting
    # 1 = Enabled (shadows under windows)
    # 0 = Disabled (no shadows under windows)
    ...
    foreach ($path in $registryPaths) {
        Write-StatusMessage -Message "Setting ListviewShadow to $newValue ($newState) in $path..." -Type Info
    ...
    Wait-OnError -ErrorMessage "Failed to toggle window shadows setting: $($_.Exception.Message)"
}