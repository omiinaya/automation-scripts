...
try {
    # Menu animation is controlled by MenuAnimation in HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced
    # This is the actual registry value that Performance Options UI reads
    $registryPath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced"
    $valueName = "MenuAnimation"
    ...
    # Toggle the setting
    # 1 = Enabled (menu animations)
    # 0 = Disabled (no menu animations)
    $newValue = if ($currentValue -eq 1) { 0 } else { 1 }
    $newState = if ($newValue -eq 1) { "enabled" } else { "disabled" }
    
    # Apply the new setting
    Write-StatusMessage -Message "Setting MenuAnimation to $newValue ($newState)..." -Type Info
