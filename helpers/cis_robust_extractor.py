...
class CISRobustExtractor:
            ...
            with pdfplumber.open(self.pdf_path) as pdf:
                for page_num in range(1, len(pdf.pages) + 1):
                    ...
                    if text:
                        ...
                        pages_data.append(
                            {"page_number": page_num, "text": "", "lines": []}
                        )
    ...
    def _parse_complex_title(self, action_type: str, title: str) -> str:
        ...
        if nested_match:
            ...
            if main_match:
                ...
                return (
                    f"{action_type} {main_content} ({nested_action} {nested_content})"
                )
            else:
    ...
    def process_pdf(self):
        ...
        try:
            ...
            self.logger.info(
                f"Successfully processed {len(self.recommendations)} recommendations"
            )
        ...
        try:
            ...
            for section_id, recommendations_data in sections.items():
                ...
                for i in range(0, len(recommendations_data), chunk_size):
                    ...
                    self.logger.info(
                        f"Saved {len(chunk)} recommendations to {output_path}"
                    )
