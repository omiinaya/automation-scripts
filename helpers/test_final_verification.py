...
def main():
    ...
    if all_passed:
        ...
        print(
            "1. Enhanced regex patterns now support action words: Ensure, Configure, Disable, Enable, Turn off, Turn on"
        )
        print(
            "2. Nested title structures are properly parsed (e.g., 'Disable IPv6 (Ensure TCPIP6 Parameter...)')"
        )
        print(
            "3. File naming uses first two parts of CIS ID (e.g., 18.6 from 18.6.19.2.1)"
        )
        print(
            "4. Section 18.6.19.2.1 is now correctly extracted and grouped with other 18.6.x recommendations"
        )
        return 0
    else:
